package mongodb

import (
	"reflect"

	"github.com/shopspring/decimal"
	"go.mongodb.org/mongo-driver/v2/bson"
	"go.mongodb.org/mongo-driver/v2/mongo"
	"go.mongodb.org/mongo-driver/v2/mongo/options"
)

// Create a new MongoDatabase with the connection string and dbname
func NewMongoDatabase(connstr string, name string) (*MongoDatabase, error) {

	// ctx := context.Background()
	opts := options.Client().ApplyURI(connstr)
	client, err := mongo.Connect(opts)
	if err != nil {
		return nil, err
	}
	// db := client.Database(dbname)
	return &MongoDatabase{client, name}, nil
}

// Create a new MongoClient with the connection string and dbname
func NewMongoDatabaseWithRegistry(connstr string, name string, reg *bson.Registry) (*MongoDatabase, error) {

	opts := options.Client().
		ApplyURI(connstr).
		SetRegistry(reg)

	client, err := mongo.Connect(opts)
	if err != nil {
		return nil, err
	}
	// db := client.Database(dbname)
	return &MongoDatabase{client, name}, nil
}

func GetBsonRegistryForDecimal() *bson.Registry {

	// 1. Create a new registry (v2 directly uses NewRegistry)
	reg := bson.NewRegistry()
	// 2. Register your custom codec directly to the registry
	// DecimalCodec must implement EncodeValue and DecodeValue
	codec := &DecimalCodec{}
	reg.RegisterTypeEncoder(reflect.TypeOf(decimal.Decimal{}), codec)
	reg.RegisterTypeDecoder(reflect.TypeOf(decimal.Decimal{}), codec)
	return reg
}

func (db MongoDatabase) db() *mongo.Database {
	return db.client.Database(db.name)
}

// collection returns the mongo collection
func (db MongoDatabase) collection(name string) *mongo.Collection {
	return db.db().Collection(name)
}
